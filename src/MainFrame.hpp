...
public:
    MainFrame() : wxFrame(nullptr, wxID_ANY, "DDS Game Lobby", wxDefaultPosition, wxSize(600, 450))
    {
        CreateStatusBar();

        waiting_panel_ = new WaitingPanel(this, [this](int screen_id)
                                          { setScreen(screen_id); });
        ...
        ttt_panel_ = new TTTGameGUI(this, waiting_panel_, [this](int screen_id)
                                    { setScreen(screen_id); }, game_user_);
        rps_panel_ = new RPSGameGUI(this, waiting_panel_, [this](int screen_id)
                                    { setScreen(screen_id); }, game_user_);
        c4_panel_ = new C4GameGUI(this, waiting_panel_, [this](int screen_id)
                                  { setScreen(screen_id); }, game_user_);
...
private:
    void setupGameSelection()
    {
        ...
        rButtonImage.LoadFile(wxT("./resources/Connect4logo.png"), wxBITMAP_TYPE_PNG);
        wxSize buttonTileSize = wxSize(125, 125);
        ...
        bottomRowSizer->Add(makeLabeledButton(C4, "Connect Four"), 0, wxALIGN_CENTER);
