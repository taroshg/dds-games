...
class Grid
{
    ...
    public:
        ...
        void setGridState(unsigned long newState);
        unsigned long getGridState();
...
unsigned long Grid::getGridState() {
    return gridstate;
}

void Grid::setGridState(unsigned long newState) {
    gridstate = newState;
    updateGridSymbols();
}

std::string Grid::getGridSymbol(int pos)
{
    std::string symbol = "a";
...
std::string Grid::getExampleGrid()
{
    return "1|2|3\n4|5|6\n7|8|9";
}
...
int Grid::powers(int num, int exponent)
{
    int result = 1;
